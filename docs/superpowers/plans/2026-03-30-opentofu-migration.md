...
```bash
export AWS_ACCESS_KEY_ID=c9f55e059a8dc11a62a85bc92138ca9a
export AWS_SECRET_ACCESS_KEY=6ea1943f27f5fe54cddfc2ecac6bd33d3f9e2b1f5569f94bd8cc67c386472ccc
export TF_VAR_cloudflare_api_token=<기존_값>
export TF_VAR_cloudflare_zone_id=<기존_값>
export TF_VAR_cloudflare_domain=zzizily.com
```

> **주의**: 실제 TF_VAR_ 값은 Terraform Cloud 또는 기존 로컬 설정에서 확인.
...
Expected: `terraform.tfstate.backup` 파일 생성됨

- [ ] **Step 3: `tofu init -migrate-state` 실행**
...
- [ ] **Step 5: `tofu plan` 실행 (변경 사항 없어야 함)**
...
Cloudflare Dashboard → R2 → `terraform-state` 버킷에서 `cloudflare-domain-zzizily/terraform.tfstate` 파일 존재 확인.
